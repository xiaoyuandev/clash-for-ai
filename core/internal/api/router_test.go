...
func TestManagedLocalGatewayProviderActivationRequiresHealthyRuntime(t *testing.T) {
	...
	handler := NewRouter(providerService, healthService, nil, manager, tooling.NewService(providerService, t.TempDir()), nil, 3456, "", gatewayHandler)
...
func newTestRouterWithLogs(t *testing.T, adapter localgateway.GatewayAdapter, runtime localgateway.RuntimeConfig) (http.Handler, *logging.Service) {
	...
	return NewRouter(providerService, healthService, loggingService, manager, tooling.NewService(providerService, t.TempDir()), nil, 3456, "", gatewayHandler), loggingService
}
