...
- [What Is The Difference Between Provider And Model?](#what-is-the-difference-between-provider-and-model)
...
### What Is The Difference Between Provider And Model?

Why does the product have both a `Provider` page and a `Model` page?

In one sentence: `Provider` manages relay services, while the `Model` page is backed by a local relay service, `LocalGateway`, that manages native model sources.
...
Both pages are related to models, but they solve different problems:

1. `Provider` answers "which API service should Codex, Claude Code, and Cursor use?"
2. `Model` answers "which native model sources should I manage locally and expose through LocalGateway?"

So another way to think about it is:

1. the `Provider` page is for tool integration and upstream switching
2. the `Model` page is for native model source management
3. `LocalGateway` appears as a selectable Provider on the `Providers` page

This split is not meant to add extra concepts. It keeps "switching relay services" separate from "managing native model sources". The former is mostly about `Base URL`, `API Key`, and the active upstream. The latter is more about model lists, capability differences, protocol compatibility, and unified local exposure.

For more detail, see [Provider vs Model](./docs/en/provider-vs-model.md).
...
- [Provider vs Model](./docs/en/provider-vs-model.md)
