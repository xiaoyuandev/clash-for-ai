...
export const glassPanelClass =
  "rounded-[28px] border [border-color:var(--border-soft)] [background:var(--panel-glass)] shadow-[var(--shadow-panel)] backdrop-blur-xl transition-[background,border-color,box-shadow] duration-300";
...
export const pillBaseClass =
  "inline-flex min-h-11 items-center rounded-full border px-4 py-2 text-xs font-medium tracking-[0.02em] backdrop-blur-md transition-[background,border-color,color] duration-300";
...
export function buttonClass(variant: "primary" | "secondary" | "danger" | "ghost" = "primary") {
  const base =
    "inline-flex min-h-11 items-center justify-center rounded-2xl border px-4 py-2.5 text-sm font-medium transition duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--accent-strong)]/55 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent disabled:cursor-not-allowed disabled:opacity-50";
  const variants = {
    primary:
...
export const inputClass =
  "min-h-11 w-full rounded-2xl border [border-color:var(--border-soft)] [background:var(--panel-input)] px-4 py-2.5 text-sm text-[color:var(--color-text)] outline-none transition placeholder:text-[color:var(--color-subtle)] focus:[border-color:var(--accent-strong)] focus:[background:var(--panel-input-focus)] focus:ring-2 focus:ring-[color:var(--accent-strong)]/20";
...
export const hintClass = "text-xs leading-6 text-[color:var(--color-muted)]";
export const metaClass = "text-sm leading-6 text-[color:var(--color-muted)]";
...
export const nestedCardClass = `${softPanelClass} p-4 sm:p-5`;
export const infoCardClass =
  "rounded-3xl border [border-color:var(--border-soft)] [background:var(--panel-solid)] p-4 backdrop-blur-md transition-[background,border-color] duration-300";
...
export const surfaceCardClass =
  "rounded-[28px] border [border-color:var(--border-soft)] [background:var(--panel-soft)] p-5 shadow-[var(--shadow-soft)] backdrop-blur-lg transition-[background,border-color,box-shadow] duration-300";

export const successNoticeClass =
  "rounded-[28px] border [border-color:var(--success-border)] [background:var(--success-soft)] p-4 text-sm text-[color:var(--success-text)] backdrop-blur-xl";

export const dangerNoticeClass =
  "rounded-[28px] border [border-color:var(--danger-border)] [background:var(--danger-soft)] p-4 text-sm text-[color:var(--danger-text)] backdrop-blur-xl";

export function selectableItemClass(active: boolean) {
  return [
    "w-full rounded-3xl border p-4 text-left transition duration-200",
    active
      ? "[border-color:var(--accent-strong)]/25 [background:color-mix(in_srgb,var(--accent-soft)_92%,transparent)] shadow-[0_16px_40px_color-mix(in_srgb,var(--accent)_14%,transparent)]"
...
export const splitLayoutClass = "grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]";
...
export const modalPanelClass = `${glassPanelClass} max-h-[calc(100vh-3rem)] w-full max-w-4xl overflow-auto p-5 sm:p-6`;
